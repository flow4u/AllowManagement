        ...
        <List sx={{ flexGrow: 1, overflow: 'auto' }}>
          {domains.map(domain => (
            ...
            >
              ...
              <Box>
                {status !== 'allowed' && (
                  <IconButton tooltip-title="Move to Allowed" size="small" onClick={() => updateStatus(domain, 'allowed')}>
                    <ArrowBack fontSize="small" />
                  </IconButton>
                )}
                {status !== 'not_reviewed' && (
                  <IconButton tooltip-title="Remove status" size="small" onClick={() => updateStatus(domain, 'not_reviewed')}>
                    <DeleteOutline fontSize="small" sx={{ opacity: 0.5 }} />
                  </IconButton>
                )}
                {status !== 'not_allowed' && (
                  <IconButton tooltip-title="Move to Not Allowed" size="small" onClick={() => updateStatus(domain, 'not_allowed')}>
